...
for (const packageName of requestedPackageNames) {
  if (!packagesByName.has(packageName)) {
    throw new Error(
      `Unknown or non-publishable package in publish list: ${packageName}`
    );
  }
}
...
const packagesToPublish =
  requestedPackageNames.length > 0
    ? requestedPackageNames.map((name) => packagesByName.get(name))
    : publishablePackages;
...
for (const { name } of packagesToPublish) {
  ...
  if (dryRun) args.push("--dry-run", "--no-git-checks");
