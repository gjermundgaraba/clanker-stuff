...
Complete. The replacement provider is the default, and `CLANKER_CODEX_PROVIDER_REPLACEMENT=0` is the explicit replay-only opt-out that restores Pi's built-in provider without losing v4/v5 checkpoint history. The superseded side-SSE network path has been deleted. Sentinel framing remains because durable replay still depends on it.

This plan supersedes the earlier architectural constraint that normal requests must remain on Pi's built-in `openai-codex` stream handler. Existing checkpoint and replay behavior remains the durable implementation baseline until the replacement provider passes the remaining live acceptance gates in this document.
...
- Keep provider replacement opt-in until the complete acceptance matrix passes. This gate passed during Phase 4, after which replacement ownership became the default.
...
Phase 4 completed on 2026-08-03. The default SSE, forced-fallback, SSE branch, genuine WebSocket branch, real-window, and mid-turn live canaries all passed against `openai-codex/gpt-5.6-sol`. The real-window run completed two compactions above 248,000 provider input tokens each; the mid-turn run completed two tool-loop compactions above 220,000 provider input tokens each. Fresh Node processes proved checkpoint resume and independent branch recovery. The old side-SSE implementation and tests were deleted; marker framing was retained as a required part of the replay-only opt-out.

Follow-up backend canaries also passed on 2026-08-03: real image input, strict structured tool output, compaction across image/tool history, a live switch from `gpt-5.6-sol` to `gpt-5.6-terra`, and ten-compaction SSE and WebSocket soaks with fresh-process replay. The WebSocket soak reused one connection with zero SSE requests.

A post-compaction request-state regression run also passed on 2026-08-03 over both SSE and WebSocket. Each transport completed two mid-turn compactions above 220,000 provider input tokens, then executed a second tool only after checkpoint installation; fresh-process replay passed, and WebSocket reused one connection with zero SSE requests.

Final repository validation passed with `pnpm check:all`: 382 unit tests, 52 integration tests, and 5 smoke tests.
