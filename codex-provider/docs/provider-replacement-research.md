...
| --- | --- | --- |
| [OpenAI Codex](https://github.com/openai/codex) | [`bb5054fe47abe73ecbbd454751066a28c89f4bb9`](https://github.com/openai/codex/tree/bb5054fe47abe73ecbbd454751066a28c89f4bb9) | Latest `main`, confirmed by both `git fetch` and the GitHub remote during this research |
...
A follow-up source refresh compared this baseline with Codex `main` at [`7750465934d97dd3cbcb3b1655d2f622744010d3`](https://github.com/openai/codex/tree/7750465934d97dd3cbcb3b1655d2f622744010d3), ten commits later. Codex now consumes `usage.codex_rollout_budget_units` for its local rollout-budget accounting, falls back to weighted token accounting when the field is absent, and rejects negative or non-finite values. Pi's `Usage` type cannot represent the field and this provider does not own a rollout-budget policy, so no provider behavior was added. Request construction, compaction, fallback, prewarm, continuation, transport, and model metadata behavior are unchanged.
...
Codex can prewarm the first WebSocket with a `response.create` request whose `generate` field is `false`. The prewarm includes the initial instructions and tools, then returns the same client session for the first real turn ([startup prewarm](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/session_startup_prewarm.rs)).
...
Inline remote V2 compaction uses the active turn's `ModelClientSession`. Standalone manual compaction creates its own client session. The request is a normal streamed Responses request containing normalized history plus a trailing `compaction_trigger`; it is not `POST /responses/compact` ([attempt construction](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/compact_remote_v2_attempt.rs), [V2 orchestration](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/compact_remote_v2.rs)).
...
The 2026-08-03 live backend included a provider-only `metadata` field on the compaction output. Codex's `ResponseItem` deserializer ignores that unknown field. The replacement provider mirrors that boundary behavior by discarding it before strict canonical checkpoint validation; persisted checkpoints still reject extra fields.
...
Fresh world-state context is inserted before the last eligible real user or non-final agent message, leaving the compaction item last ([retention and placement](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/compact_remote.rs)).
...
Codex's `/models` response carries much more than a picker label. Relevant fields include model/base instructions, model-authored collaboration and permission messages, context and maximum window sizes, the auto-compaction limit, effective-window percentage, `comp_hash`, default reasoning effort, supported reasoning efforts, service tiers, text verbosity, tools/modalities, and Responses Lite support ([model schema](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/protocol/src/openai_models.rs)).

Codex refreshes `GET /models?client_version=...`, uses ETags, and normally treats cached entries as fresh for 300 seconds ([model manager](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/models-manager/src/manager.rs), [cache](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/models-manager/src/cache.rs), [endpoint](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/codex-api/src/endpoint/models.rs)). The default automatic compaction limit is 90% of the resolved context window.

Before sampling, Codex compacts when a model's non-empty `comp_hash` changes or when switching to a smaller model whose usable context is already exceeded. It records prior model settings so that it can compact with the previous model first ([turn transition](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/session/turn.rs)).

### 5. Context windows have explicit generations

Codex assigns UUIDv7 identities to the first, current, and previous context windows. It advances the window only after compaction is installed and sends the current identity in `x-codex-window-id` ([auto-compact window](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/state/auto_compact_window.rs)).
...
Codex computes structured snapshots and diffs for model instructions, personality, token-budget guidance, realtime mode, repository instructions, permissions, collaboration mode, environment state, apps, plugins, deferred tools, extension contributions, and multi-agent mode ([session world state](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/session/world_state.rs), [world-state diffing](https://github.com/openai/codex/blob/bb5054fe47abe73ecbbd454751066a28c89f4bb9/codex-rs/core/src/context/world_state/mod.rs)).
