...
- `@clanker-extensions/codex-provider` with Node `>=24`, public Pi peers, repository metadata, and `pi.extensions`.
...
After appending `../../code/priv/clanker-extensions/codex-provider` as the final global package, both initial load and reload resolve:

```text
Resolved 23 extensions
Final extension: /Users/gg/code/priv/clanker-extensions/codex-provider/index.ts
```
...
| 23 | `codex-provider/index.ts` | `context`, `before_provider_headers`, `before_provider_request`, `session_before_compact` |
...
A temporary trusted project fixture with one project top-level extension and one project package resolves both before the global codex-provider package; the target remains final across reload. This is evidence for the current Pi behavior, not a guarantee for future configuration.
...
```bash
node codex-provider/audit-local-order.ts /Users/gg/code/priv/clanker-extensions
```
