...
```bash
node codex-provider/scripts/evaluate-agents.ts --smoke
```
...
```bash
node codex-provider/scripts/evaluate-agents.ts --dry-run --repetitions 2
```
...
```bash
node codex-provider/scripts/evaluate-agents.ts \
  --model gpt-5.6-sol \
  --reasoning high \
  --repetitions 3 \
  --timeout-minutes 20
