...
pnpm exec vitest run --project unit codex-provider/tests/phase-zero.test.ts
pnpm exec vitest run --project integration codex-provider/tests/phase-zero.integration.test.ts
pnpm exec ultracite check codex-provider/spike.ts codex-provider/tests
pnpm exec oxfmt --check codex-provider/spike.ts codex-provider/tests codex-provider/spike-results.md
pnpm typecheck
```
