...
- `tests/harness/extension-host.ts`
- `tests/harness/tui.ts`
...
- `tests/harness/agent-session.ts`

Any test that imports `tests/harness/agent-session.ts`, including tests of the harness itself, must be named `*.integration.test.ts`.
...
- `tests/harness/extension-smoke.ts`

Any test that imports `tests/harness/extension-smoke.ts`, including tests of the harness itself, must be named `*.smoke.test.ts`.
...
TUI-heavy interactive flows usually belong in unit/TUI tests with `extension-host` and `tests/harness/tui.ts`.

For generic `ui.custom(...)` mechanics, prefer the shared driver in `tests/harness/tui.ts` over package-local wiring. Keep package-local helpers only when they add package semantics such as fixture builders, domain-specific queued results, or package-language assertions.
...
- Tests importing `tests/harness/agent-session.ts` must be `*.integration.test.ts`.
- Tests importing `tests/harness/extension-smoke.ts` must be `*.smoke.test.ts`.
