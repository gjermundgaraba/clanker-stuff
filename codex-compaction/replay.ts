...
export const frameContiguousBaseline = <T>(
  ...
  if (
    framedSegment.some(
      (message, index) =>
        canonicalJsonValue(message) !== baselineValues[segmentOffset + index]
    )
  ) {
    return { kind: "missing" };
  }
...
const buildReplacement = (
  ...
  let remaining = tokenBudget;
  const retainedReversed: RealUserInputItem[] = [];

  for (const item of users.toReversed()) {
    if (remaining === 0) {
      continue;
    }
...
export const buildCheckpointReplacement = (
  provableUserItems: readonly unknown[],
  newCompaction: unknown,
  ...
  const users = provableUserItems.map((item, index) => {
    const user = parseRealUserInputItem(item, `provableUserItems[${index}]`);
    return {
      ...user,
  ...
  return buildReplacement(users, newCompaction, tokenBudget);
};
...
export const buildTransientCheckpointReplacement = (
  ...
  return buildReplacement(
    provableUserItems.map((item, index) =>
      parseRealUserInputItem(item, `provableUserItems[${index}]`)
    ),
    newCompaction,
    tokenBudget
  );
