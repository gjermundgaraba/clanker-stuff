...
_PI_EVENT_GUARD = (
    ...
    "process.exitCode=settled&&outcome===true&&!failed?0:1;});"
)
...
def _text(content: Any, *, thinking: bool = False) -> str:
    ...
    for block in content:
        ...
        if (
            ...
            not thinking and block_type == "text" and isinstance(block.get("text"), str)
        ):
...
def _compaction_extra(event: dict[str, Any]) -> dict[str, Any]:
    ...
    checkpoint_value = details.get("checkpoint")
    checkpoint = checkpoint_value if isinstance(checkpoint_value, dict) else {}
    protocol = checkpoint.get("protocol")
    succeeded = (
        bool(result) and not event.get("errorMessage") and not event.get("aborted")
    )
    state = (
        "succeeded" if succeeded else "aborted" if event.get("aborted") else "failed"
    )
    tokens_before = result.get("tokensBefore", entry.get("tokensBefore"))
    if succeeded or isinstance(checkpoint_value, dict):
...
def convert_pi_events(
    ...
    *,
    ...
    manifest = validate_manifest(pi_evals)
    steps: list[Step] = []
    calls: dict[str, Step] = {}
    totals: dict[str, int | float] = {
        "cache_read": 0,
        ...
        if event_type == "compaction_end":
            ...
            append_step(
                ...
                observation=Observation(
                    results=[ObservationResult(content=compaction_extra["state"])]
                ),
...
class PiEval(Pi):
    ...
    async def run(
        ...
        marker = json.dumps(
            {
                "controlled_compaction": compact_before,
                "index": index,
    ...
    def populate_context_post_run(self, context: AgentContext) -> None:
        ...
        marker_indexes = [
            ...
            if event.get("type") == "harbor_instruction"
        ]
        if marker_indexes:
            marker_index = marker_indexes[-1]
            current_events = events[marker_index:]
            if events[marker_index].get("controlled_compaction") is True:
                compaction = next(
                    (
                        event
                        for event in reversed(events[:marker_index])
                ...
                if compaction is None:
                    raise ValueError(
                        "controlled compaction marker has no preceding compaction"
                    )
                current_events = [compaction, *current_events]
        else:
        ...
        for event in current_events:
            ...
            elif event.get("type") == "compaction_end" and isinstance(
