...
from fixtures.trials import MANIFEST, completed_trajectory, write_trial
...
class TrialsTest(TestCase):
    def test_completed_row_uses_verified_manifest_reward_usage_and_timings(
        ...
        with TemporaryDirectory() as directory:
            ...
            [row] = trials.rows(Path(directory))
            self.assertEqual(
                (
                    row["trial"],
                    row["status"],
    ...
    def test_completed_oracle_allows_unavailable_trajectory_usage(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                all(
                    row[key] is None
                    for key in (*trials.METER_KEYS, *trials.COMPACTION_KEYS)
                )
            )
        ...
        with TemporaryDirectory() as directory:
            ...
            [row] = trials.rows(Path(directory))
            self.assertEqual(row["status"], "completed")
            self.assertTrue(all(row[key] is None for key in trials.METER_KEYS))
    ...
    def test_errors_use_the_latest_available_cumulative_trajectory(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            [row] = trials.rows(root)
            self.assertEqual(
                (row["status"], row["input"], row["compaction_attempts"]),
                ("errored", 5, 0),
            )

    def test_incomplete_trajectory_usage_and_compactions_fail_independently(
        self,
    ) -> None:
