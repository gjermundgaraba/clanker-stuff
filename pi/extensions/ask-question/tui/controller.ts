...
type Field = { kind: "custom" | "note"; question?: string; option?: string };
export class QuestionnaireView {
  ...
  private detail?: { title: string; text: string; markdown: boolean };
  private editor?: { component: Editor; field: Field; initial: string };
  private hint = "";
  ...
  private async saveField(force = false): Promise<void> {
    ...
    if (field.kind === "custom" && field.question)
      await this.change({ type: "custom", question: field.question, text });
    else await this.change({ type: "note", question: field.question, option: field.option, text });
  ...
  private async choose(index: number, toggle = false): Promise<void> {
    ...
    const draft = this.item.draft!.answers[q.id];
    if (!option) {
  ...
  private async processInput(data: string): Promise<void> {
    ...
    if (action === "key:n") {
      ...
      this.edit(
        { kind: "note", question: q.id, option: option?.id },
