...
export const runAskQuestionPrompt = async (
  ...
  return await ctx.ui.custom<AskQuestionFlowResult>((tui, theme, keybindings, done) => {
    ...
    const moveCursor = (questionIndex: number, delta: number) => {
      ...
      state.cursor = Math.max(0, Math.min(question.options.length - 1, state.cursor + delta));
      hint = "";
      tui.requestRender();
    };
    ...
    const handleEditorSave = (value: string) => {
      ...
      state.textByOptionIndex[editMode.optionIndex] = bounded.length === 0 ? undefined : bounded;
      closeEditor();
    };
    ...
    const openOptionTextEditor = (questionIndex: number, optionIndex: number) => {
    ...
    const openNoteEditorForCurrentSelection = (questionIndex: number) => {
      const { question, state } = questionSessions[questionIndex];
      const optionIndex = question.multiSelect ? state.cursor : [...state.selectedIndexes][0];
    ...
    const handleSubmitTabInput = (intent: DecodedIntent) => {
      ...
      if (result === undefined) {
        setHint(`Incomplete: ${missingQuestionHeaders(questionSessions).join(", ")}`);
        return;
      }
      finish(result);
    };
    ...
    const handleInput = (data: string) => {
      ...
      handleQuestionInput(questionIndex, questionSessions[questionIndex].question, intent);
    };
