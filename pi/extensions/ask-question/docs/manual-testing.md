...
- With long Markdown context, verify PageUp/PageDown reaches every line in both directions on short supported terminals (for example, 18 rows), including after resizing. Scroll using PageUp/PageDown, then move the option highlight: the focused option must come into view without selecting it. Verify the question footer shows the configured scrolling keys, including remapped bindings. Open and close a preview, then save and discard an option note: the highlighted answer must be visible on return, without selecting it. Check revision reason, form context and question context appear in that order, and questions without context have no context marker or shortcut.
