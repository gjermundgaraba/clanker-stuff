...
describe("codex-skills package", () => {
  ...
  it("does not statically expose orchestration through an omitted skill filter", async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "codex-skills-filter-"));
  ...
  it("packs the conditional discovery runtime and vendored skill", () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "codex-skills-pack-"));
    const tarball = path.join(tempRoot, "codex-skills.tgz");
