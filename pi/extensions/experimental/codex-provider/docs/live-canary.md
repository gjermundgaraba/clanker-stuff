...
This mode calibrates natural text, adds bounded hidden history so Pi can choose a valid cut point, and performs two real tool loops with host compaction enabled. In each round the model must call `context_filler` exactly once. Its result fills at least 80% of the real context and crosses Pi's configured threshold, forcing `session_before_compact` before the next model response. The canary requires one Pi-owned lifecycle `CompactionEntry` per round and rejects inline checkpoint entries. The model must then call `post_compaction_probe` exactly once; the tool rejects an early call by recording how many checkpoints existed when it ran. This proves the post-compaction request still contains the current tool schemas and system instructions. Both checkpoints must have phase `mid-turn`, and their windows must form one monotonic replacement chain.
...
The model keeps its native declared context window; this run does not force the small estimator window used by the default synthetic canary. A missing picker entry is accepted only when the selected model carries catalog visibility `hide`; listed models must match their declared window. One happy path verifies project instructions, provider/API identity, the active model's startup tool profile (direct tools, hybrid direct tools with Code Mode, or Code Mode only), `/code-mode` behavior, a strict manual checkpoint over that tool history, a non-persisting `/codex-provider` status request, and fresh-process tool availability for the restored profile plus opaque checkpoint recall. Explicit catalog profiles must refuse a toggle; optional profiles exercise both modes. Direct-only profiles skip the Code Mode execution phase. The installed `/tools` command must also be present, proving the provider is running alongside the tools extension.
