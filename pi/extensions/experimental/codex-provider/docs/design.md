...
Live model metadata is authoritative; the pinned fallback catalog covers offline startup, including an explicit Astra profile with Responses Lite, V2 collaboration, Ultra at `xhigh`, a 272,000-token context window, and 128,000-token maximum output. Hidden models retain their metadata for explicit model selection and resume but stay out of the model picker; a live `visibility: list` makes them available there. Persisted remote catalogs are bound to the ChatGPT account that produced them and are invalidated before another account can observe their entitlements or model policy. A catalog-observed account change also invalidates account-scoped usage cache and in-flight usage publication. The lightning status appears only when the selected model supports fast mode.
...
Pi normally omits its loaded-skill catalog when `read` is unavailable. When the provider's direct or Code Mode tool set suppresses `read`, the provider restores that catalog with guidance naming the active file-capable tool. The loader follows the current tool inventory after policy refresh, even when the event's prompt options still describe the previous inventory. It does not discover or load any additional skills.
