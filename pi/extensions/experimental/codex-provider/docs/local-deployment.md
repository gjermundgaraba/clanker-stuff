...
This package is approved only for a controlled installation of the supported Pi baseline, currently 0.85.0, where its resolved `index.ts` is the final enabled extension. The provider runtime is always on, and the audit rejects every other Pi SDK or executable version.
...
Privacy warning: Radius-backed `/share` in Pi 0.85.0 uploads the full system prompt and every active tool's name, description, and schema.
