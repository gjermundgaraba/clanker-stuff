...
const countLines = async (path: string): Promise<number> => {
  ...
  for await (const chunk of createReadStream(path)) {
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- SAFETY: Node streams erase the chunk type; this private createReadStream has no encoding and therefore yields Buffers.
