...
type ToolRenderContext = Parameters<
  NonNullable<ToolDefinition<TUnsafe<unknown>, unknown, object>["renderCall"]>
>[2];
...
const PolledSessionSchema = Type.Object({ session_id: Type.Number() });
...
const traceOutcomes = (traces: RuntimeToolTrace[]): Map<string, Outcome> => {
  ...
  for (const trace of traces) {
    ...
    const polledSession =
      trace.name === "write_stdin" && Value.Check(PolledSessionSchema, input)
        ? input.session_id
        : undefined;
...
const formatExecCall = (args: Static<typeof ExecArgsSchema> | undefined, theme: Theme): string => {
  const title = theme.fg("toolTitle", theme.bold("Exec"));

  if (args === undefined) return `${title} ${theme.fg("error", "[invalid arg]")}`;
...
const actionLabel = (kind: Kind, args: Static<typeof WaitArgsSchema> | undefined): Action =>
  kind === "exec" ? "Exec" : args?.terminate === true ? "Terminate" : "Wait";
...
const callContent = (
  ...
  if (kind === "exec") {
    return preview(
      () =>
        new Text(formatExecCall(Value.Check(ExecArgsSchema, args) ? args : undefined, theme), 0, 0),
      expanded,
  ...
  const title = theme.fg("toolTitle", theme.bold(actionLabel(kind, valid ? args : undefined)));
...
const renderCodeModeResult = (
  ...
  const action = actionLabel(
    kind,
    kind === "wait" && Value.Check(WaitArgsSchema, context.args) ? context.args : undefined,
  );
