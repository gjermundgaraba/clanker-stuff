...
type Animation = { seconds: number; frames: number[]; still: number };
// SAFETY: Generated metadata is checked below for dimensions, timing and scalar values.
const manifest = JSON.parse(
  ...
  fps: number;
  seed: string;
  animations: Record<string, Record<string, Record<string, Animation>>>;
  inks: Record<string, Record<string, string>>;
  rubik: Animation;
};
...
describe("shape spinner", () => {
  ...
  it.each(["rubik", ...Object.keys(manifest.animations)])("selects %s", async (shape) => {
    const { ctx, host, setWorkingIndicator } = setup();
  ...
  it.each(Object.keys(manifest.inks))(
  ...
  it.each(["dark", "light"])("selects %s background", async (background) => {
    const { ctx, host, setWorkingIndicator } = setup();
  ...
  it("previews all shapes and a spacer-free mapping without changing state", async () => {
    ...
    for (const color of Object.keys(manifest.inks)) {
      expect(preview).toContain(`${color} ${glyph(manifest.animations.orb[color].dark.still)}`);
    }
