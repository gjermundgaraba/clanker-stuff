...
interface ActiveSide {
  ...
  disposed: boolean;
  draft: string;
  presentation: SidePresentation | undefined;
  unread: boolean;
...
export const createSideController = (
  pi: ExtensionAPI,
  createConversation = createSideConversation,
) => {
  ...
  const openSide = async (args: string, ctx: ExtensionContext): Promise<void> => {
    ...
    try {
      conversation = await createConversation(ctx, pi.getThinkingLevel());
    } catch (error) {
