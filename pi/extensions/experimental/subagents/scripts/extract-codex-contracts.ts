...
const checkout =
  process.env.CODEX_CHECKOUT ?? path.join(homedir(), ".cache/checkouts/github.com/openai/codex");
...
const functionBody = (input: string, name: string, sourcePath = SPEC_PATH): string => {
...
const stringConstant = (input: string, name: string): string => {
  const value = new RegExp(`(?:pub )?const ${name}: &str = "(?<value>[^"]+)";`, "u").exec(input)
    ?.groups?.value;
...
const sortedUnique = (values: readonly string[]): string[] => [...new Set(values)].toSorted();

const propertyKeys = (body: string): string[] =>
  [...body.matchAll(/"(?<key>[a-z][a-z0-9_]*)"\.to_string\(\),/gu)].flatMap((match) =>
    match.groups?.key === undefined ? [] : [match.groups.key],
  );

const requiredPropertySets = (body: string): string[][] =>
  [...body.matchAll(/"required": \[(?<properties>[^\]]+)\]/gu)].map((match) =>
    [...(match.groups?.properties ?? "").matchAll(/"(?<key>[a-z_]+)"/gu)].flatMap((property) =>
      property.groups?.key === undefined ? [] : [property.groups.key],
    ),
  );

const without = (values: readonly string[], excluded: readonly string[]): string[] =>
  values.filter((value) => !excluded.includes(value));

const toolFactories = [
  ...specSource.matchAll(/pub fn (?<factory>create_[a-z0-9_]+)\([^)]*\) -> ToolSpec \{/gu),
].map((match) => {
  ...
  const name = /name: "(?<name>[a-z_]+)"\.to_string\(\),/u.exec(body)?.groups?.name;
...
const namespaceMerge = functionBody(planSource, "merge_into_namespaces", PLAN_PATH);
...
const v1Tools = sortedUnique(
  toolFactories.flatMap(({ protocol, name }) => (protocol === "v1" ? [name] : [])),
);

const v2Tools = sortedUnique(
  toolFactories.flatMap(({ protocol, name }) => (protocol === "v2" ? [name] : [])),
);
...
const defaultBoolean = (name: string): boolean => {
  const value = new RegExp(`${name}: (?<value>true|false)`, "u").exec(configSource)?.groups?.value;
...
const exposeModelOverrides = defaultBoolean("expose_spawn_agent_model_overrides");
...
const parsedCatalog: unknown = JSON.parse(source(CATALOG_PATH, CATALOG_COMMIT));
...
const fixture = {
  ...
  v2: {
    namespace: stringConstant(configSource, "DEFAULT_MULTI_AGENT_V2_TOOL_NAMESPACE"),
...
const rendered = execFileSync("vp", ["fmt", "--stdin-filepath", fixturePath], {
  encoding: "utf-8",
  input: `// Generated by scripts/extract-codex-contracts.ts. Do not edit.
export const codexContractFixture = ${JSON.stringify(fixture)} as const;
`,
});
...
if (mode === "--write") {
  ...
  if (existing !== rendered) {
    throw new Error("Codex contract fixture is stale; rerun extractor with --write");
  }
}
