...
`wait_agent` resolves with the final status it observes for a target. A completed or errored turn reports its own final status even when queued work is immediately promoted in the same completion transaction, matching Codex's per-thread status channel, so an empty status accompanies only a timeout or a controller reset; a later `wait_agent` observes only later transitions. A non-final transition (an interrupted turn, or a completion whose attempt was superseded) does not wake a registered wait; the wait stays parked until a final status appears, the controller resets, or the deadline expires. `close` and a controller reset wake a registered wait without a status, so it re-reads the live state (final for `close`, empty for a reset).
...
V2 excludes models explicitly declaring `multiAgentVersion: "disabled"`; V1 children such as Luna remain eligible in a V2 tree, without gaining collaboration tools. Unknown-model errors list the same five suggestions. Explicit or role-configured reasoning is checked against the final selected model after role precedence. Inherited effort retains the existing Pi runtime behavior. Neither an invalid model nor invalid explicit effort silently falls back to the parent.
