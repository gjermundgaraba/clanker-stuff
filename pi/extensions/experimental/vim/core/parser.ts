...
function target(
  ...
  if (object && objects.test(rest))
    return { motion: "object", target: rest.slice(1), around: rest.startsWith("a") };
  ...
  if (first && "fFtT".includes(first) && single(rest.slice(1)))
    return { motion: first, target: rest.slice(1) };
