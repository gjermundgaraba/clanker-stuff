...
Requires Pi 0.85 or newer. Composes with this repository's history and dollah-skills editor factories. Foreign editors not derived from `CustomEditor` are left unchanged and unavailable. `CustomEditor` subclasses with an incompatible rendered top border are also unavailable. A compatible border spans the editor width and starts and ends with rule characters. At widths below five cells, structural judgment is deferred; narrowing an established compatible editor does not revoke availability. An extension that replaces the editor without composing the previous factory can displace the border host; load this host after such extensions, or disable the incompatible editor.
