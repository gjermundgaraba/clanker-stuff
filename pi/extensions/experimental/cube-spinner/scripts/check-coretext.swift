#!/usr/bin/env swift  // Run on macOS: swift scripts/check-coretext.swift [font.ttf [cube.json]]
...
guard arguments.count <= 2 else {
  fail("usage: swift check-coretext.swift [font.ttf [cube.json]]")
}
let assets = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
  .deletingLastPathComponent().appendingPathComponent("assets")
let fontURL =
  arguments.isEmpty
  ? assets.appendingPathComponent("CubeSpinner.ttf")
  : URL(fileURLWithPath: arguments[0])
let metadataURL =
  arguments.count == 2
  ? URL(fileURLWithPath: arguments[1])
  : fontURL.deletingLastPathComponent().appendingPathComponent("cube.json")

do {
  let manifest = try JSONDecoder().decode(Manifest.self, from: Data(contentsOf: metadataURL))
  guard !manifest.frames.isEmpty, manifest.frames.allSatisfy({ $0.count == 2 }) else {
    fail("expected nonempty two-codepoint frames in \(metadataURL.path)")
  }
  let codepoints = Set(manifest.frames.map { $0[0] }).sorted()
